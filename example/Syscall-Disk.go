...
func main() {
	NtAllocateVirtualMemory, err := ghostevasion.GetSysidDisk("NtAllocateVirtualMemory")
	if err != nil {
		panic(err)
	}
	...
	NtWriteVirtualMemory, err := ghostevasion.GetSysidDisk("NtWriteVirtualMemory")
	if err != nil {
		panic(err)
	}
	...
	var Thread uintptr
	NtCreateThreadEx, err := ghostevasion.GetSysidDisk("NtCreateThreadEx")
	if err != nil {
		panic(err)
	}
	...
	Time := -(INFINITE)
	NtWaitForSingleObject, err := ghostevasion.GetSysidDisk("NtWaitForSingleObject")
	if err != nil {
		panic(err)
	}
